...
兼容桥现在将目标楼层的正文保存错误直接传给正在等待的 MVU 提交，并在成功写入后刷新上下文缓存。原有 15 秒提交启发式仍保留，此修复不代表事件完成屏障已经全面实现。

用户提供的 3.1.1 卡与 3.27 预设在本地脚本回放中覆盖两个开场、每个开场两轮合成输入与应答；执行实际 Schema 和维护脚本，检查变量提交、状态栏标记与旧楼层不变。3.27 预设不含启用的嵌入脚本；该回放不覆盖预设提示词和正则应用。初次回放时 ADB 无设备。随后两个修复包已通过 ADB 覆盖安装；用户于 2026-09-07 明确反馈测试完毕、没有问题。该确认对应本次报错场景，不扩展为全部兼容接口验收。
...
8. 日志容量回收已落地（提交后不再写 `.previous`、每目标 `.commits` 裁剪至最近 8 条、启动 sweep 清扫孤儿与过期 `.original`、备份不再打包 journal）；多存储实例的跨进程锁及所有存储对象的统一事务仍待补齐。保留日志不能代替完整数据迁移和恢复说明/工具。

这些是当前源码可见或缺少证据的缺口，不是已通过项目。本计划完成前仍应继续处理，不得用目标卡的成功样例代替通用范围验收。

## 恢复与保留规则

聊天/普通 JSON 的日志在对应数据根的 `.tellev-writes`；扩展设置有自己的日志根。`.pending` 描述目标、原值校验、负载和操作收据；`.payload` 保存要写入的字节；`.state` 与 `.commits` 记录修订和去重（`.commits` 每目标只保留最近 8 条，仅用于 operationId 幂等重放检测）；`.original` 保留首版原文件，删除目标时清理、超过 30 天由启动 sweep 清理。不再写 `.previous` 副本；历史遗留的 `.previous` 与孤儿条目由 sweep 回收。
