# tellev 1.5.1 beta 测试包

此构建作为独立应用 `app.tellev.beta` 安装，显示名称为 `tellev Beta`，版本名为
`1.5.1-beta.1`。它可以与正式版并存，数据和设置互不共享。
...
发布前将 APK 重命名为 `tellev-1.5.1-beta.1.apk`，同时计算并公布 SHA-256。

## GitHub 与论坛分发

1. 在 `braze-taffo/tellev` 创建标签和 Release：`v1.5.1-beta.1`。
2. Release 标题使用 `tellev 1.5.1 Beta 1（贴吧公开测试）`。
...
4. 上传 `tellev-1.5.1-beta.1.apk`，并在正文列出 SHA-256、Android 12+、测试期限、
   HTTP 明文风险以及反馈帖地址。
...
- 内置默认预设及缺省运行时回退使用 1,000,000 上下文、131,072（128 Ki）最大输出；从旧测试版升级首次进入时会提示检查当前预设。
