...
> 公开版本以 GitHub Releases 中发布的 APK 为准。当前版本 **v1.0.2**。
...
- 聊天：流式生成、停止生成、重试、swipes、编辑、删除、收藏、附件、推理块（reasoning）渲染、TavernHelper 风格的 HTML 片段渲染
- 角色：列表、标签、导入、编辑、复制、导出
- 世界书：条目管理、搜索、激活预览、常驻/排他激活
...
- 宏展开：`{{char}}`、`{{user}}`、日期/时间、random、变量、扩展提供的宏
- Instruct 模式与上下文模板（Context Template）
- 世界书插入规则（depth / position / 优先级 / 插入顺序）
- 群聊发言人排序
- Token 预算感知的上下文裁剪
