...
- 正式版（`master` / `app.tellev`）仅使用远程生图服务，不包含本地推理、MNN 核心或模型导入功能。
- 聊天内每次生图都可选择已配置的 ComfyUI 或 NovelAI 引擎。
- NovelAI 的「总结当前场景」会输出经校验的英文图像 tag，格式不合格时自动重试一次。
- 修复生成图片消息可能被前端富文本分支遮住的问题。
- 本地 SD1.5 / MNN 推理、模型管理和前台保活仅在 `mnn-image-gen` 分支提供，包名为 `app.tellev.mnn`，使用独立签名和预发布渠道。
